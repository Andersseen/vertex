...
export class PreviewPanelComponent implements OnDestroy {
  ...
  constructor() {
    effect(() => {
      ...
      this.logs.set([]);
      this.joinedLogs.set('');
      // Preview is created fresh on each startPreview() call using the current fs.
  ...
  private async startPreview(): Promise<void> {
    ...
    this.preview = new WebContainerPreview(iframe, {
      fs,
      onPhase: (phase, message) => this.statusLabel.set(phaseLabel(phase, message)),
      onLog: ({ chunk }) => {
    ...
    try {
      const session = await this.preview.start(config);
...
function phaseLabel(phase: PreviewPhase, message?: string): string {
  switch (phase) {
    ...
    case 'mount':
      return message ?? 'Mounting files…';
    case 'install':
      return 'Installing dependencies (first run may take a while)…';
    case 'run':
      return message ? `Running ${message}…` : 'Running…';
    case 'dev':
